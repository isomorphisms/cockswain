...
collector=${1:-"$root/bin/cockswain-authority-collect"}
work=$(mktemp -d)
...
grep -F "authorized_scope_sha256	$scope_sha" "$work/refreshed-same-scope.tsv" >/dev/null
...
printf '%s\n' 'live task-context authority collector acceptance passes'
